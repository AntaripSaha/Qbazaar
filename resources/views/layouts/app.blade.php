    ...
    <body class="sb-nav-fixed">
		...
		<nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
            <div class="container-fluid">
				<a class="navbar-brand text-md-center" href="{{ url('/dashboard') }}">{{ get_option('site_title', config('app.name')) }}</a>
				...
				<ul class="navbar-nav ml-auto ml-md-0">
					<li class="nav-item dropdown animate-dropdown">
						...
						<div class="dropdown-menu dropdown-menu-right" aria-labelledby="userDropdown">
							<a class="dropdown-item" href="{{ route('profile.edit') }}"><i class="icofont-ui-user"></i> {{ _lang('Profile Settings') }}</a>
							<a class="dropdown-item" href="{{ route('profile.change_password') }}"><i class="icofont-exchange"></i> {{ _lang('Change Password') }}</a>
							...
							<a class="dropdown-item" href="{{ route('logout') }}"><i class="icofont-logout"></i> {{ _lang('Logout') }}</a>
						</div>
					</li>
				</ul>
...
<!-- translator scripts End-->
        ...
        <!-- App js -->
        <script src="{{ asset('backend/assets/js/scripts.js?v=1.0') }}"></script>
		...
		(function($) {
			...
			@foreach ($errors->all() as $error)
			    @if ($loop->first)
					$("#main_alert > span.msg").html("<i class='icofont-close-line-squared-alt'></i> {{ $error }} ");
					$("#main_alert").addClass("alert-danger").removeClass("alert-success");
				@else
                    $("#main_alert > span.msg").append("<br><i class='icofont-close-line-squared-alt'></i> {{ $error }} ");
				@endif
